...
## Loading data
Initially, suppose we have `vessel_info.csv` file generated by some other tool, 
...
First, let's create a directory named `dataset` and put
...
After executing these codes, the structure of the `dataset` directory 
becomes
```
dataset
+-- conf
...
following some strategies. Say, if we want to concat the contents in all
`plan_{exp}/{VesselID}/berthing.csv` into another file 
`plan_{exp}/oops_yet_another_berthing_info_file.csv` which has the same fields
as these `berthing.csv` files, the recipe can be automatically generated 
by
```
...
quick_recipe("concat", 
    ...
    dishes=[plan_{exp}/oops_yet_another_berthing_info_file.csv"], 
    axis=0)
```
